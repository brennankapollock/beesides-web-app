...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchProfileTimeout = (promise: Promise<void>, timeoutMs = 2000): Promise<void> => {
    return Promise.race([
      promise,
    ...
    const fetchPromise = async () => {
      try {
        ...
        const userProfile: AuthUser = {
          ...
          username: data?.username || user.user_metadata?.username || fallbackProfile.username,
        ...
        console.log("Current user updated in context. Is currentUser set?", !!userProfile);
      } catch (error) {
        ...
        console.log("Setting minimal user profile after error:", minimalProfile);
  ...
  const register = async (
    ...
    try {
      ...
      console.log("Registration complete, user is now authenticated and ready for onboarding");
      return;
    } catch (error) {
